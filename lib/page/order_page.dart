...
class _OrderPageState extends State<OrderPage> {
  ...
  void _completeOrder() {
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: const Text('Nama Pemilik Order'),
          content: TextField(
            controller: ownerNameController,
            decoration: const InputDecoration(
                hintText: 'Masukkan Nama Pemilik Pesanan '),
          ),
          actions: [
            TextButton(
              onPressed: () {
            ...
            ElevatedButton(
              onPressed: () {
                ...
                if (ownerName.isEmpty) {
                  _showErrorSnackBar('Nama pemilik tidak boleh kosong');
                } else {
                  Navigator.of(context).pop();
                  _saveOrderAndNavigateToHistory(ownerName);
                }
              },
  ...
  void _saveOrderAndNavigateToHistory(String ownerName) {
    _uploadOrderHistoryToFirebase(ownerName);
  ...
  void _uploadOrderHistoryToFirebase(String ownerName) async {
    FirebaseFirestore firestore = FirebaseFirestore.instance;
    try {
      await firestore.collection('orderHistory').add({
        'ownerName': ownerName,
                ...
                Flexible(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      Map<String, dynamic> product = products[index];
                      return Card(
                        color: const Color.fromARGB(255, 233, 212, 170),
                        margin: const EdgeInsets.symmetric(
                            vertical: 8.0, horizontal: 16.0),
                        elevation: 4,
                        child: Padding(
